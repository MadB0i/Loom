...
import type { Canvas, ImageElementData } from '../crdt/canvas'
...
export function ImageBlock({ canvas, element, tool }: ImageBlockProps) {
  ...
  const handleResizeMove = (event: React.PointerEvent) => {
    ...
    canvas.updateElement(element.id, { width, height: width * ratio })
  }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="absolute -bottom-1.5 -right-1.5 h-3 w-3 cursor-nwse-resize rounded-sm border border-border bg-surface-2 opacity-0 transition-opacity duration-150 group-hover:opacity-100"
