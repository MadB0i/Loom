...
import { Hand, Image as ImageIcon, PenLine, Plus } from 'lucide-react'
...
export function CanvasToolbar({
  ...
  const handleAddText = () => {
    ...
    const x = (centerX - positionX) / scale + offset
    const y = (centerY - positionY) / scale + offset
    setCascade((c) => c + 1)
    onAddText(x, y)
  }
    ...
    >
      <div className="pointer-events-auto flex items-center gap-1 rounded-xl border border-white/10 bg-white/[0.06] p-1.5 shadow-2xl shadow-black/50 backdrop-blur-xl">
        ...
        <button
          type="button"
          onClick={onAddImage}
          title="Add image (paste or drag-and-drop an image file also work)"
